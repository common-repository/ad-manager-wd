...
$wd_ads_stats_from=isset($_POST['wd_ads_stats_from']) ? sanitize_text_field($_POST['wd_ads_stats_from']) : date('Y-m').'-01';
$wd_ads_stats_to=isset($_POST['wd_ads_stats_to']) ? sanitize_text_field($_POST['wd_ads_stats_to']) : date('Y-m-d');
...
<input onclick="" type="submit" value="submit" class='button button-primary button-large' />
    ...
    <input type="hidden" id="export_scv" name="export_scv" value="0" />
    <input type="hidden" id="" name="dates" value="<?php echo esc_attr(json_encode($dates)) ?>">
    <input type="hidden" id="" name="impressions_by_date" value="<?php echo esc_attr(json_encode($impressions_by_date)) ?>">
    <input type="hidden" id="" name="clicks_by_date" value="<?php echo esc_attr(json_encode($clicks_by_date)) ?>">
    <input type="hidden" id="" name="title" value="<?php echo esc_attr( $title.'_stat') ?>">
    <input type="hidden" id="" name="range" value="<?php echo esc_attr($wd_ads_stats_from.' to '.$wd_ads_stats_to) ?>">

    <button onclick="jQuery('#export_scv').val(1); this.form.submit()" type="button" value="Export SCV" class='button button-primary button-large'>Export SCV</button>
